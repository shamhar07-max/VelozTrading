...
export function clerkProxyMiddleware(): RequestHandler {
  ...
  if (!secretKey) {
    console.warn("[clerk-proxy] CLERK_SECRET_KEY is not set — proxy disabled");
    return (_req, _res, next) => next();
  }
  ...
  const keyType = pubKey.startsWith("pk_live_")
    ...
    ? "development (pk_test_) ⚠️  switch to pk_live_ before going live"
    : "unknown";
  console.info(`[clerk-proxy] active — key type: ${keyType}`);
  console.info(`[clerk-proxy] routing ${CLERK_PROXY_PATH} → ${CLERK_FAPI}`);
